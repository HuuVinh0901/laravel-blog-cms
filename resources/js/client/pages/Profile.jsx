...
const ProfileBase = ({ isOwner, userId: userIdProp }) => {
    ...
    const [menuOpen, setMenuOpen] = useState(null); // Trạng thái cho menu tác vụ
    ...
    const handleMenuToggle = (postId, e) => {
        e.preventDefault(); // Ngăn chặn sự kiện click lan tỏa
        e.stopPropagation(); // Ngăn chặn sự kiện kích hoạt Link
        setMenuOpen(menuOpen === postId ? null : postId); // Toggle menu
    };

    const handleEditPost = (postId, e) => {
        e.preventDefault();
        e.stopPropagation();
        navigate(`/edit-post/${postId}`); // Giả định route chỉnh sửa bài viết
        setMenuOpen(null);
    };
    ...
    const handleDeletePost = (postId, e) => {
        e.preventDefault();
        e.stopPropagation();
        // Giả định API xóa bài viết
        if (window.confirm('Bạn có chắc muốn xóa bài viết này?')) {
            // Gọi API xóa: deletePost(postId)
